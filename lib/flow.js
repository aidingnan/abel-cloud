...
class Manager {
  ...
  createOrPush(userId, task) {
    // 插入任务
    this.task.set(userId, (this.task.get(userId) || []).push(task))
    console.log('插入任务完成', this.task.get(userId))
  }

  delete(userId, task) {
    let value = this.task.get(userId)
    console.log(value)
    this.task.set(userId, value.splice(value.indexOf(task), 1))
    console.log('删除任务完成', this.task.get(userId))
  }
}
...
class Flow extends Transform {
  ...
  _transform(chunk, encoding, callback) {
    ...
    if (this.limit && !this.useHighFlow && this.part > 307200) {
      // 限速且没有高速流量
  ...
  async clean() {
    manager.delete(this.userId, this)
