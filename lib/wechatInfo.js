...
class WechatInfo {
  ...
  oauth2UserInfo(type, code, callback) {
    ...
    _this.accessToken(code, function (error, data) {
      ...
      let { refresh_token, access_token, openid } = data

      _this.refreshToken(refresh_token, function (err, refresh) {
        if (err)
          return callback(err)
        ...
        _this.userInfo(refresh.access_token, data.openid, function (er, userInfo) {
          ...
          callback(null, Object.assign(userInfo, { refresh_token, access_token, openid, type }))
        })
      })
